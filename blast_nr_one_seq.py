...
def BLASTseqsNR():
	...
	Path(f'BLAST_records_renamed').mkdir(parents=True, exist_ok=True)
	...
	for file in os.listdir('LSGF_NTD'):
		if file.endswith('fa'):
			...
			if not os.path.exists(f'BLAST_records/{all_seqs[0].id}'):
				print(f'\nBLAST-ing Sequences in {file} \n')
				with open(f'BLAST_records/{all_seqs[0].id}', 'w') as o:
					results = qblast(program = 'blastx', sequence = all_seqs[0].seq, database = 'nr', hitlist_size = 8, format_type = 'XML')
					o.write(results.read())
					results.close
			else:
				print(f'\n{file} BLAST results already exist \n')
...
def parseXML():
	...
	with open('BLASTnrOutliers.tsv', 'w') as o:
		o.write("Seq Title\tQuery Name\tQuery Length\tAlignment Title\tAlignment ID\tAlignment Def\teValue\tCoverage\tPercent_ID\tAlignment_Length\tOG")
		for xml_file in os.listdir('BLAST_records_renamed'):
			#if xml_file.startswith(clade):
			result_handle = open('BLAST_records_renamed/' + xml_file)
			blast_records = NCBIXML.parse(result_handle)
			for rec in blast_records:
				for alignment in rec.alignments:
					for hsp in alignment.hsps:
						o.write('\n'+str(xml_file) +'\t'+ str(rec.query_id) + '\t' + str(rec.query_length) + '\t' + str(alignment.title) + '\t' + str(alignment.hit_id) + '\t' + str(alignment.hit_def) + '\t' + str(hsp.expect) + '\t' + str(hsp.align_length / rec.query_length) + '\t' + str(hsp.identities/ hsp.align_length) +'\t'+ str(hsp.align_length) + '\t' + str(xml_file.split("_")[-1]))
